  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できない時' do
      ...
      it 'passwordが存在してもpassword_confirmationが空では登録できない' do
        ...
        @user.valid?
        expect(@user.erros.full_messages).to include("Password confirmation doesn't match Password")
      end
      it 'nicknameが7文字以上では登録できない' do
        @user.nickname = 'aaaaaa'
        @user.valid?
        expect(@user.erros.full_messages).to include("Nickname is too long (maximum is 6 characters)")
      end
